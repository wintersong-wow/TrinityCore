...
(
SELECT a.ID, b.Effect_1 AS effect, a.ItemVisual, a.Name_Lang_enUS AS DBCNAME, b.Name_Lang_enUS FROM
db_spellitemenchantment_12340 a LEFT JOIN db_spell_12340 b ON a.ID = b.EffectMiscValue_1 WHERE a.ItemVisual > 0 UNION
SELECT a.ID, b.Effect_2 AS effect, a.ItemVisual, a.Name_Lang_enUS AS DBCNAME, b.Name_Lang_enUS FROM
db_spellitemenchantment_12340 a LEFT JOIN db_spell_12340 b ON a.ID = b.EffectMiscValue_2 WHERE a.ItemVisual > 0 UNION
SELECT a.ID, b.Effect_3 AS effect, a.ItemVisual, a.Name_Lang_enUS AS DBCNAME, b.Name_Lang_enUS FROM
db_spellitemenchantment_12340 a LEFT JOIN db_spell_12340 b ON a.ID = b.EffectMiscValue_3 WHERE a.ItemVisual > 0 ORDER BY
ID ASC ) x WHERE effect IN (53, 54, 92) OR effect IS NULL GROUP BY ItemVisual ORDER BY ItemVisual ASC;
*/
...
const char* Transmogrification::GetSlotName(uint8 slot, WorldSession* /*session*/)
{
    ...
    switch (slot)
    {
        case EQUIPMENT_SLOT_HEAD:
            return "Head"; // session->GetTrinityString(LANG_SLOT_NAME_HEAD);
        case EQUIPMENT_SLOT_SHOULDERS:
            return "Shoulders"; // session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
        case EQUIPMENT_SLOT_BODY:
            return "Shirt"; // session->GetTrinityString(LANG_SLOT_NAME_BODY);
        case EQUIPMENT_SLOT_CHEST:
            return "Chest"; // session->GetTrinityString(LANG_SLOT_NAME_CHEST);
        case EQUIPMENT_SLOT_WAIST:
            return "Waist"; // session->GetTrinityString(LANG_SLOT_NAME_WAIST);
        case EQUIPMENT_SLOT_LEGS:
            return "Legs"; // session->GetTrinityString(LANG_SLOT_NAME_LEGS);
        case EQUIPMENT_SLOT_FEET:
            return "Feet"; // session->GetTrinityString(LANG_SLOT_NAME_FEET);
        case EQUIPMENT_SLOT_WRISTS:
            return "Wrists"; // session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
        case EQUIPMENT_SLOT_HANDS:
            return "Hands"; // session->GetTrinityString(LANG_SLOT_NAME_HANDS);
        case EQUIPMENT_SLOT_BACK:
            return "Back"; // session->GetTrinityString(LANG_SLOT_NAME_BACK);
        case EQUIPMENT_SLOT_MAINHAND:
            return "Main hand"; // session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
        case EQUIPMENT_SLOT_OFFHAND:
            return "Off hand"; // session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
        case EQUIPMENT_SLOT_RANGED:
            return "Ranged"; // session->GetTrinityString(LANG_SLOT_NAME_RANGED);
        case EQUIPMENT_SLOT_TABARD:
            return "Tabard"; // session->GetTrinityString(LANG_SLOT_NAME_TABARD);
        default:
            return NULL;
    }
}
...
std::string Transmogrification::GetItemLink(Item* item, WorldSession* session)
{
    ...
    oss << "|c" << std::hex << ItemQualityColors[temp->Quality] << std::dec << "|Hitem:" << temp->ItemId << ":"
        ...
        << item->GetItemSuffixFactor() << ":" << (uint32)item->GetOwner()->GetLevel() << "|h["
        << GetItemName(item, session) << "]|h|r";
...
std::string Transmogrification::GetItemLink(uint32 entry, WorldSession* session)
{
    ...
    const ItemTemplate* temp = sObjectMgr->GetItemTemplate(entry);
    std::ostringstream oss;
    oss << "|c" << std::hex << ItemQualityColors[temp->Quality] << std::dec << "|Hitem:" << entry
        << ":0:0:0:0:0:0:0:0:0|h[" << GetItemName(temp, session) << "]|h|r";
...
TransmogResult Transmogrification::CannotTransmogrifyItemWithItem(Player* player, ItemTemplate const* target,
                                                                  ItemTemplate const* source, bool ever)
{
    ...
    // commented out because display can be same as different item entries can have same look but you want to display
    // different item in inspect etc if (source->DisplayInfoID == target->DisplayInfoID)
    //     return false;
    ...
    if (source->SubClass != target->SubClass && !IsBowOrGunOrCrossbow(target))
    {
        ...
        if (source->Class == ITEM_CLASS_WEAPON &&
            !IsMeleeWeapon(target)) // Wands and thrown should not be transmogrifiable with other items like swords etc.
            return TransmogResult_WeaponTypesDontMatch;
    }
    ...
    {
        if (source->Class == ITEM_CLASS_WEAPON &&
            !((IsBowOrGunOrCrossbow(target) || AllowMixedInventoryTypes ||
...
static bool IsFitToSpellRequirements(SpellInfo const* spellInfo, ItemTemplate const* proto)
{
    (void)spellInfo;
    (void)proto;
    // bool const isEnchantSpell = spellInfo->HasEffect(SPELL_EFFECT_ENCHANT_ITEM) ||
    // spellInfo->HasEffect(SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) ||
    // spellInfo->HasEffect(SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC); if (spellInfo->EquippedItemClass != -1) // -1 == any
    // item class
    //{
...
TransmogResult Transmogrification::CannotTransmogrifyItemWithEnchant(Player* player, ItemTemplate const* destination,
                                                                     uint32 enchant, bool ever)
{
    (void)player;
    (void)ever;
    ...
    // if (!Ignore && source->EnchantmentCondition && !player->EnchantmentFitsRequirements(source->EnchantmentCondition,
    // -1))
    //    return ??;

    // if (!IgnoreReqLevel && source->MinLevel > player->GetLevel())
    //     return TransmogResult_TooLowLevelPlayer;
    // if (!IgnoreReqLevel && source->RequiredSkillID > 0 && source->RequiredSkillRank >
    // player->GetSkillValue(source->RequiredSkillID))
    //     return TransmogResult_TooLowSkill;
    // if (destination->ItemLevel < spellInfo->BaseLevel || (destination->RequiredLevel && destination->RequiredLevel <
    // spellInfo->BaseLevel))
...
bool Transmogrification::IsBowOrGunOrCrossbow(ItemTemplate const* itemTemplate)
{
    ...
    return Class == ITEM_CLASS_WEAPON &&
           (SubClass == ITEM_SUBCLASS_WEAPON_BOW || SubClass == ITEM_SUBCLASS_WEAPON_GUN ||
...
bool Transmogrification::IsMeleeWeapon(ItemTemplate const* itemTemplate)
{
    ...
    return Class == ITEM_CLASS_WEAPON &&
           (SubClass == ITEM_SUBCLASS_WEAPON_AXE || SubClass == ITEM_SUBCLASS_WEAPON_AXE2 ||
            ...
            SubClass == ITEM_SUBCLASS_WEAPON_POLEARM || SubClass == ITEM_SUBCLASS_WEAPON_SWORD ||
            SubClass == ITEM_SUBCLASS_WEAPON_SWORD2 || SubClass == ITEM_SUBCLASS_WEAPON_STAFF ||
            SubClass == ITEM_SUBCLASS_WEAPON_FIST || SubClass == ITEM_SUBCLASS_WEAPON_DAGGER ||
...
void Transmogrification::LoadConfig(bool /*reload*/)
{
    ...
    AllowHeirloom  = sConfigMgr->GetBoolDefault("Transmogrification.AllowHeirloom", false);
...
void Transmogrification::LoadEnchants()
{
    ...
    for (uint32 spellid = 0; spellid < sSpellMgr->GetSpellInfoStoreSize(); ++spellid)
    {
        ...
        {
            if (effect.Effect == SpellEffects::SPELL_EFFECT_ENCHANT_ITEM ||
                effect.Effect == SpellEffects::SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY ||
                effect.Effect == SpellEffects::SPELL_EFFECT_ENCHANT_HELD_ITEM)
            {
...
TransmogResult Transmogrification::TrySetPendingTransmog(Player* player, uint32 slot, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("custom.transmog",
                     "Transmogrification::Transmogrify - {} ({}) tried to transmogrify invalid slot ({}) with {}.",
                     player->GetName(), player->GetGUID().ToString(), slot, entry);
    ...
    if (hasTemplate)
    {
        ...
        {
            TC_LOG_DEBUG("custom.transmog",
                         "Transmogrification::Transmogrify - {} ({}) tried to transmogrify slot {} with a non-existant "
                         "item entry {}.",
                         player->GetName(), player->GetGUID().ToString(), slot, entry);
            return TransmogResult_NonexistantTransmog;
        }
    ...
    {
        TC_LOG_DEBUG("custom.transmog",
                     "Transmogrification::Transmogrify - {} ({}) tried to transmogrify slot {} with entry {}, but the "
                     "slot was empty.",
                     player->GetName(), player->GetGUID().ToString(), slot, entry);
    ...
    if (hasTemplate)
    {
        if (TransmogResult res =
                CannotTransmogrifyItemWithItem(player, itemTransmogrified->GetTemplate(), itemtemplate, false))
        {
            TC_LOG_DEBUG(
                "custom.transmog",
                "Transmogrification::Transmogrify - {} ({}) failed CannotTransmogrifyItemWithItem ({} with {}).",
                player->GetName(), player->GetGUID().ToString(), itemTransmogrified->GetEntry(), entry);
            return res;
        }
    }
...
TransmogResult Transmogrification::TrySetPendingEnchant(Player* player, uint32 slot, uint32 entry)
{
    ...
    {
        if (TransmogResult res =
                CannotTransmogrifyItemWithEnchant(player, itemTransmogrified->GetTemplate(), entry, false))
        {
            TC_LOG_DEBUG(
                "custom.transmog",
                "Transmogrification::Transmogrify - {} ({}) failed CannotTransmogrifyItemWithEnchant ({} with {}).",
                player->GetName(), player->GetGUID().ToString(), itemTransmogrified->GetEntry(), entry);
            return res;
        }
    }
...
static std::map<SkillType, uint32> SkillToClassMask = {
    ...
    {SKILL_LEATHER, classesToClassMask({CLASS_PALADIN, CLASS_WARRIOR, CLASS_DEATH_KNIGHT, CLASS_HUNTER, CLASS_SHAMAN,
                                        CLASS_DRUID, CLASS_ROGUE})},
    ...
    {SKILL_AXES,
     classesToClassMask({CLASS_WARRIOR, CLASS_SHAMAN, CLASS_PALADIN, CLASS_HUNTER, CLASS_DEATH_KNIGHT, CLASS_ROGUE})},

    {SKILL_2H_SWORDS, classesToClassMask({CLASS_WARRIOR, CLASS_PALADIN, CLASS_HUNTER, CLASS_DEATH_KNIGHT})},
    {SKILL_SWORDS, classesToClassMask({
                       CLASS_WARRIOR,
                       CLASS_PALADIN,
                       CLASS_HUNTER,
                       CLASS_DEATH_KNIGHT,
    ...
    {SKILL_MACES, classesToClassMask({CLASS_WARRIOR, CLASS_SHAMAN, CLASS_PALADIN, CLASS_DRUID, CLASS_DEATH_KNIGHT,
                                      CLASS_ROGUE, CLASS_PRIEST})},
    ...
    {SKILL_STAVES, classesToClassMask({CLASS_WARRIOR, CLASS_WARLOCK, CLASS_SHAMAN, CLASS_PRIEST, CLASS_MAGE,
                                       CLASS_HUNTER, CLASS_DRUID})},
    ...
    {SKILL_DAGGERS, classesToClassMask({CLASS_WARRIOR, CLASS_WARLOCK, CLASS_SHAMAN, CLASS_ROGUE, CLASS_PRIEST,
                                        CLASS_MAGE, CLASS_HUNTER, CLASS_DRUID})},
...
{
    // select * from item_template where class not in (2,4) and inventorytype in
    // (1,34,5,6,7,8,9,10,13,14,1516,17,19,20,21,22,23,25,26,28)
    if (proto->Class != ITEM_CLASS_ARMOR && proto->Class != ITEM_CLASS_WEAPON)
        return TransmogResult_InvalidItemType;

    if (proto->InventoryType != INVTYPE_HEAD && proto->InventoryType != INVTYPE_SHOULDERS &&
        proto->InventoryType != INVTYPE_BODY && proto->InventoryType != INVTYPE_CHEST &&
        ...
        proto->InventoryType != INVTYPE_HANDS && proto->InventoryType != INVTYPE_WEAPON &&
        proto->InventoryType != INVTYPE_SHIELD && proto->InventoryType != INVTYPE_RANGED &&
        proto->InventoryType != INVTYPE_CLOAK && proto->InventoryType != INVTYPE_2HWEAPON &&
        proto->InventoryType != INVTYPE_TABARD && proto->InventoryType != INVTYPE_ROBE &&
        proto->InventoryType != INVTYPE_WEAPONMAINHAND && proto->InventoryType != INVTYPE_WEAPONOFFHAND &&
        proto->InventoryType != INVTYPE_HOLDABLE && proto->InventoryType != INVTYPE_THROWN &&
        proto->InventoryType != INVTYPE_RANGEDRIGHT && proto->InventoryType != INVTYPE_RELIC)
        return TransmogResult_InvalidItemType;
...
void Transmogrification::AddToCollection(Player* player, Item* item)
{
    ...
    if (uint32 transmog = AddItemVisualToCollection(player, item))
        ChatHandler(player->GetSession())
            .PSendSysMessage(AddToCollectionMessageFmt, GetItemLink(transmog, player->GetSession()).c_str());
}
...
void Transmogrification::RemoveAllTransmogrifications(Player* player)
{
    for (Item* item : GetEquippedItems(player))
    {
        ...
        player->SetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (item->GetSlot() * 2), 0,
...
bool Transmogrification::HasPendingTransmog(Player* player, uint8 slot, Item** retItem, uint32* retPending,
                                            uint32* retCurrent)
{
...
bool Transmogrification::HasPendingEnchant(Player* player, uint8 slot, Item** retItem, uint32* retPending,
                                           uint32* retCurrent)
{
    Item* item = GetEquippedItem(player, slot);
    if (!item)
