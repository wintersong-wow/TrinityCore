...
class TC_GAME_API Transmogrification
{
  ...
  public:
    ...
    static const std::unordered_map<uint32, std::string>
        enchant_visual_to_name; // contains all enchant visuals (not enchants) that should be viewable in menus
    ...
    std::unordered_map<uint32, std::vector<SpellInfo const*>>
        enchant_to_spells; // contains all enchants. Some spell lists can be empty
    std::unordered_map<uint32, std::vector<uint32>>
        spell_to_enchants; // does not contain all enchants as all of them dont come from spells
