...
{
    bool isBouncingFar =
        (m_spellInfo->HasAttribute(SPELL_ATTR4_AREA_TARGET_CHAIN) || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_NONE ||
...
void Spell::_cast(bool skipCheck)
{
    ...
    if (Unit* unitCaster = m_caster->ToUnit())
        if (m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET))
            if (Creature* pet = ObjectAccessor::GetPet(*m_caster, unitCaster->GetPetGUID()))
    ...
    for (SpellEffectInfo const& spellEffectInfo : m_spellInfo->GetEffects())
    {
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                if (skillId != SKILL_NONE && skillId != SKILL_HERBALISM && skillId != SKILL_MINING && skillId != SKILL_WOODCUTTING && (!m_selfContainer || ((*m_selfContainer) != this)))
                {
