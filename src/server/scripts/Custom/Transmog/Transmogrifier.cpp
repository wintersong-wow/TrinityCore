...
- use item displays instead of item entries - similar menu selection like on retail where a display lists multiple names
related to it
- make sure the system allows enchants only for correct weapons - Warrior will not have cloth items added to transmog
list on retail
- More settings, for example restrict enchants somehow?
- For now, all enchants are enabled always. On retail some enchants are enabled by default and others come from items
that bosses drop
...
enum TransmogGossipSender
{
    ...
    SENDER_PAGINATED_EQUIPMENT_END =
        SENDER_PAGINATED_EQUIPMENT_START + ((EQUIPMENT_SLOT_END - EQUIPMENT_SLOT_START) * max_pages),
...
struct PageData
{
    PageData(uint32 page, uint32 total_actions)
    {
        ...
        current_page = page + 1;
        total_pages =
            static_cast<decltype(total_pages)>(std::ceil(total_actions / static_cast<double>(max_page_items)));
    }
    uint32 start;
    uint32 end;
    uint32 count;
...
struct PendingItemsCheck : public BasicEvent
{
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        if (!pending_transmogs.empty())
        {
            ...
            {
                if (lastNotSavedAlert < GameTime::GetGameTimeMS() &&
                    player->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_GOSSIP) == nullptr)
                {
  ...
  public:
    ...
    class TransmogAI : public ScriptedAI
    {
      public:
        ...
        static void AddItemTransmog(Player* player, Creature* creature, uint32 base, uint32 page, uint32 action,
                                    std::string const& name)
        {
            ...
            if (itementry == RemovePending)
                AddGossipItemFor(
                    player, GOSSIP_ICON_MONEY_BAG,
                    "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tRemove pending transmogrification",
                    SENDER_REVERT_TRANSMOG, base + page);
            else if (itementry == InvisibleEntry)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Ability_Vanish:30:30:-18:0|tHide item", base + page,
                                 InvisibleEntry);
            else if (itementry == NormalEntry)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Spell_Holy_Restoration:30:30:-18:0|tRestore original look",
                                 base + page, NormalEntry);
            else
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 Transmogrification::instance().GetItemIcon(itementry, 30, 30, -18, 0) + name,
                                 base + page, itementry);
        }

        static void AddItemEnchant(Player* player, Creature* creature, uint32 base, uint32 page, uint32 action,
                                   std::string const& name)
        {
            (void)creature;
            ...
            if (enchantentry == RemovePending)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tRemove pending enchant",
                                 SENDER_REVERT_ENCHANT, base + page);
            else if (enchantentry == InvisibleEntry)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Ability_Vanish:30:30:-18:0|tHide enchant", base + page,
                                 InvisibleEntry);
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 std::string("|TInterface/ICONS/INV_Enchant_FormulaGood_01:30:30:-18:0|t") + name,
                                 base + page, enchantentry);
        }
        ...
        static bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
            {
                if (const char* slotName = Transmogrification::instance().GetSlotName(slot, session))
                {
                    Item* newItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, slot);
                    {
                        ...
                        bool hasPending =
                            Transmogrification::instance().HasPendingTransmog(player, slot, nullptr, &pending);
                        ...
                        if (entry == InvisibleEntry)
                            ...
                            icon = entry ? Transmogrification::instance().GetItemIcon(entry, 30, 30, -18, 0) :
                                           Transmogrification::instance().GetSlotIcon(slot, 30, 30, -18, 0);
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, icon + std::string(slotName) + status,
                                         SENDER_SHOW_ITEMS, slot);
                    }
                    ...
                    for (uint32 slotindex = 0; slotindex < ENCHANT_SLOTS.size(); ++slotindex)
                    {
                        if (slot == ENCHANT_SLOTS[slotindex])
                        {
                            ...
                            bool hasPending =
                                Transmogrification::instance().HasPendingEnchant(player, slot, nullptr, &pending);
                            ...
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                             icon + std::string(slotName) + " enchant" + status, SENDER_SHOW_ENCHANTS,
                                             slotindex);
                        }
                    }
                }
            ...
            if (Transmogrification::instance().EnableSets)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|tManage sets",
                                 SENDER_PRESET_LIST, 0);
            {
                ...
                AddGossipItemFor(
                    player, GOSSIP_ICON_MONEY_BAG,
                    "|TInterface/ICONS/INV_Enchant_EssenceCosmicGreater:30:30:-18:0|tSave pending transmogrifications",
                    SENDER_TRANSMOG_SAVE, 0,
                    "All equipped items with pending transmogrifications will save their appearance and are made "
                    "soulbound, non-refundable and non-tradeable.\n\nDo you wish to continue?",
                    copperCost, false);
                AddGossipItemFor(
                    player, GOSSIP_ICON_MONEY_BAG,
                    "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tCancel pending transmogrifications",
                    SENDER_REVERT_ALL_TRANSMOG, 0, "Cancel pending transmogrifications from all equipped items?", 0,
                    false);
            }
            AddGossipItemFor(
                player, GOSSIP_ICON_MONEY_BAG,
                "|TInterface/ICONS/Spell_Holy_Restoration:30:30:-18:0|tRestore original look",
                SENDER_REMOVE_ALL_TRANSMOG, 0,
                "Equipped items are reverted to their original look with a pending change.\n\nDo you want to continue?",
                0, false);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                   ...
                                   uint32 boxMoney = 0)
        {
            ...
            switch (sender)
            {
                ...
                case SENDER_SHOW_ENCHANTS:
                {
                    ...
                    uint32 page      = 0;
                    ShowTransmogs(player, creature, SENDER_PAGINATED_ENCHANT_START, TRANSMOG_TYPE_ENCHANT, slotindex,
                                  page);
                }
                break;
                ...
                case SENDER_PRESET_LIST: // Presets menu
                {
                    ...
                    if (Transmogrification::instance().EnableSetInfo)
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                         "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tHow sets work",
                                         SENDER_PRESET_INFO, 0);
                    for (PresetMapType::const_iterator it = player->presetMap.begin(); it != player->presetMap.end();
                         ++it)
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                         "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|t" + it->second.name,
                                         SENDER_PRESET_VIEW, it->first);
                    if (player->presetMap.size() < Transmogrification::instance().MaxSets)
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                         "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tSave set ",
                                         SENDER_CODE_PRESET_SAVE, 0, "Insert set name and click accept", 0, true);
                        ...
                        for (auto&& v : it->second.data)
                        {
                            switch (std::get<AppearanceType>(v))
                            {
                                case TRANSMOG_TYPE_ITEM:
                                    if (TransmogResult res = Transmogrification::instance().TrySetPendingTransmog(
                                            player, std::get<uint8>(v), std::get<uint32>(v)))
                                    {
                                        session->SendNotification("%s", CanTransmogrifyResultMessage(res));
                                    }
                                    break;
                                case TRANSMOG_TYPE_ENCHANT:
                                    if (TransmogResult res = Transmogrification::instance().TrySetPendingEnchant(
                                            player, std::get<uint8>(v), std::get<uint32>(v)))
                                    {
                                        session->SendNotification("%s", CanTransmogrifyResultMessage(res));
                                    }
                ...
                case SENDER_PRESET_VIEW: // view preset
                {
                    ...
                    for (auto&& v : it->second.data)
                    {
                        switch (std::get<AppearanceType>(v))
                        {
                            case TRANSMOG_TYPE_ITEM:
                                AddGossipItemFor(
                                    player, GOSSIP_ICON_MONEY_BAG,
                                    Transmogrification::instance().GetItemIcon(std::get<uint32>(v), 30, 30, -18, 0) +
                                        Transmogrification::instance().GetItemLink(std::get<uint32>(v), session),
                                    sender, action);
                                break;
                            case TRANSMOG_TYPE_ENCHANT:
                            {
                                ...
                                if (auto* source = sSpellItemEnchantmentStore.LookupEntry(enchant))
                                {
                                    ...
                                    {
                                        if (const char* slotName =
                                                Transmogrification::instance().GetSlotName(std::get<uint8>(v), session))
                                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                                             "|TInterface/ICONS/INV_Scroll_05:30:30:-18:0|t" +
                                                                 std::string(slotName) + std::string(" enchant: ") +
                                                                 it->second,
                                                             sender, action);
                                    }
                                }
                    ...
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                     "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tUse set", SENDER_PRESET_USE,
                                     action,
                                     "Using a set will remove pending transmogrifications and add the "
                                     "transmogrifications from the set as pending.\nDo you wish to continue?\n\n" +
                                         it->second.name,
                                     0, false);
                    AddGossipItemFor(
                        player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tRename set",
                        SENDER_CODE_PRESET_RENAME, action, "Insert new name\n\nOld name: " + it->second.name, 0, true);
                    AddGossipItemFor(
                        player, GOSSIP_ICON_MONEY_BAG,
                        "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tDelete set",
                        SENDER_PRESET_DELETE, action, "Are you sure you want to delete " + it->second.name + "?", 0,
                        false);
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tBack..",
                                     SENDER_PRESET_LIST, 0);
                    SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                ...
                default: // from 0 to EQUIPMENT_SLOT_END-1 TrySetPendingTransmog
                {
                    if (isbetween(sender, SENDER_PAGINATED_EQUIPMENT_START, SENDER_PAGINATED_EQUIPMENT_END))
                    {
                        ...
                        ShowTransmogs(player, creature, SENDER_PAGINATED_EQUIPMENT_START, TRANSMOG_TYPE_ITEM, slot,
                                      page);
                    }
                    if (isbetween(sender, SENDER_PAGINATED_ENCHANT_START, SENDER_PAGINATED_ENCHANT_END))
                    {
                        ...
                        ShowTransmogs(player, creature, SENDER_PAGINATED_ENCHANT_START, TRANSMOG_TYPE_ENCHANT,
                                      slotindex, page);
                    }
                }
                break;
            ...
            switch (sender)
            {
                case SENDER_CODE_PRESET_SAVE:
                {
                    ...
                    if (name.length() < 1 || name.length() > 255)
                    {
                        player->GetSession()->SendNotification(
                            "%s", CanTransmogrifyResultMessage(TransmogResult_TooLongSetName));
                        OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                        return true;
                    }
                    ...
                    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
                    {
                        ...
                        uint32 displayedenchant =
                            player->GetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (slot * 2), 0);
                    ...
                    if (items.empty())
                    {
                        // no displays to save
                        player->GetSession()->SendNotification(
                            "%s", CanTransmogrifyResultMessage(TransmogResult_NoTransmogrifications));
                        OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                        return true;
                    }
                    ...
                    if (player->presetMap.size() < Transmogrification::instance().MaxSets)
                    {
                        for (uint8 i = 0; i < Transmogrification::instance().MaxSets;
                             ++i) // should never reach over max
                        {
                    ...
                    if (presetID >= Transmogrification::instance().MaxSets)
                    {
                        ...
                        player->GetSession()->SendNotification("%s",
                                                               CanTransmogrifyResultMessage(TransmogResult_AtMaxSets));
                        OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                        return true;
                    }
                ...
                case SENDER_CODE_PRESET_RENAME:
                {
                    ...
                    if (player->presetMap.find(presetID) == player->presetMap.end())
                    {
                        ...
                        player->GetSession()->SendNotification(
                            "%s", CanTransmogrifyResultMessage(TransmogResult_NonexistantSet));
                        OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                        return true;
                    }
        ...
        static void BuildAndSendGossipMenu(Player* player, Creature* creature, uint32 slot, uint32 base, uint32 page,
                                           std::vector<std::tuple<uint32, std::string, std::string>> const& actions,
                                           AppearanceType transmogtype)
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                             std::string("|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|t") +
                                 Transmogrification::instance().GetSlotName(slot, player->GetSession()) + " - Page " +
                                 std::to_string(data.current_page) + "/" + std::to_string(data.total_pages),
                             base + page, 0);
            if (data.next)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Spell_ChargePositive:30:30:-18:0|tNext page", base + page + 1, 0);
            if (data.prev)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Spell_ChargeNegative:30:30:-18:0|tPrevious page", base + page - 1,
                                 0);
            if (data.ismainpage)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tShow main menu", SENDER_SHOW_MAIN_MENU, 0);
        ...
        static void ShowTransmogs(Player* player, Creature* creature, TransmogGossipSender basesender,
                                  AppearanceType transmogtype, uint32 slot_or_slotindex, uint32 page)
        {
            ...
            switch (transmogtype)
            {
                case TRANSMOG_TYPE_ITEM:
                {
                    for (uint32 appearance : player->transmogrification_appearances[transmogtype])
                    {
                        ...
                        if (source && !Transmogrification::instance().CannotTransmogrifyItemWithItem(player, target,
                                                                                                     source, true))
                            actions.emplace_back(
                                appearance,
                                Transmogrification::instance().GetItemLink(source->ItemId, player->GetSession()),
                ...
                case TRANSMOG_TYPE_ENCHANT:
                {
                    ...
                    static const auto try_add_enchant = [](std::set<uint32>& added_enchant_visuals,
                                                           std::vector<Action>& actions, Player* player,
                                                           const ItemTemplate* target, uint32 enchant)
                    {
            ...
            std::sort(actions.begin(), actions.end(),
                      [&](Action const& a, Action const& b) { return std::get<2>(a) < std::get<2>(b); });
        ...
        static void GetTransmogSlotAndPage(uint32 sender, TransmogGossipSender basesender, uint32& slot_or_slotindex,
                                           uint32& page)
        {
