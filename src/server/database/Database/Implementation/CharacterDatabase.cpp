...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_DEL_CHARACTER_BAN, "DELETE cb FROM character_banned cb INNER JOIN characters c ON c.guid = cb.guid WHERE c.account = ?", CONNECTION_ASYNC);
    /* Transmog Queries */
    PrepareStatement(CHAR_DEL_ACCOUNT_TRANSMOG, "DELETE FROM custom_account_transmog WHERE accountid = ?",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_ACCOUNT_TRANSMOG,
                     "SELECT `type`, entry FROM custom_account_transmog WHERE accountid = ? ORDER BY entry",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ACCOUNT_TRANSMOG,
                     "INSERT INTO custom_account_transmog (`accountid`, `type`, `entry`) VALUES (?, ?, ?)",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_TRANSMOG_SETS, "DELETE FROM `custom_transmogrification_sets` WHERE owner = ?",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_TRANSMOG_SETS,
    ...
    PrepareStatement(CHAR_SEL_MAILITEMS,
                     ...
                     "randomPropertyId, durability, playedTime, text, item_guid, itemEntry, ii.owner_guid, m.id, 0, "
                     "transmog, enchant FROM mail_items mi INNER JOIN mail m ON mi.mail_id = m.id LEFT JOIN "
                     "item_instance ii ON mi.item_guid = ii.guid WHERE m.receiver = ?",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_AUCTION_ITEMS,
                     "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, ii.flags, enchantments, "
                     "randomPropertyId, durability, playedTime, text, itemguid, itemEntry, 0, 0, 0, transmog, enchant "
    ...
    PrepareStatement(CHAR_REP_ITEM_INSTANCE,
                     "REPLACE INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, "
                     "duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, "
                     "transmog, enchant, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                     CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE,
                     "UPDATE item_instance SET itemEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, "
                     "count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, "
                     "durability = ?, playedTime = ?, text = ?, transmog = ?, enchant = ? WHERE guid = ?",
                     CONNECTION_ASYNC);
    // @tswow-end
