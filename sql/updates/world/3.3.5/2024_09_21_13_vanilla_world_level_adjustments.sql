...
UPDATE `creature_template` SET `minlevel` = 59, `maxlevel` = 59 WHERE `entry` IN (18253);  -- TODO: This is currently overriding some SmartAI and needs a custom fix
UPDATE `creature_template` SET `minlevel` = 58, `maxlevel` = 58 WHERE `entry` IN (18255);

/* Arena NPCs */
 UPDATE `creature_template` SET `minlevel` = 60, `maxlevel` = 60
-- , `ScriptName`='npc_ipp_tbc' 
 WHERE `entry` IN
...
-- Squish the existing expansion npc levels into vanilla brackets.

-- Min Level
update `creature_template` set `minlevel` = 60 where `minlevel` = 70;
update `creature_template` set `minlevel` = 60 where `minlevel` = 80;
update `creature_template` set `minlevel` = 61 where `minlevel` = 71;
update `creature_template` set `minlevel` = 61 where `minlevel` = 81;
update `creature_template` set `minlevel` = 62 where `minlevel` = 72;
update `creature_template` set `minlevel` = 62 where `minlevel` = 82;
update `creature_template` set `minlevel` = 63 where `minlevel` = 73;
update `creature_template` set `minlevel` = 63 where `minlevel` = 83;
-- All the other creatures who are not potentially elites down to 60.
update `creature_template` set `minlevel` = 60 where `minlevel` > 60 and `minlevel` not in (61,62,63);

-- Max Level
update `creature_template` set `maxlevel` = 60 where `maxlevel` = 70;
update `creature_template` set `maxlevel` = 60 where `maxlevel` = 80;
update `creature_template` set `maxlevel` = 61 where `maxlevel` = 71;
update `creature_template` set `maxlevel` = 61 where `maxlevel` = 81;
update `creature_template` set `maxlevel` = 62 where `maxlevel` = 72;
update `creature_template` set `maxlevel` = 62 where `maxlevel` = 82;
update `creature_template` set `maxlevel` = 63 where `maxlevel` = 73;
update `creature_template` set `maxlevel` = 63 where `maxlevel` = 83;
-- All the other creatures who are not potentially elites down to 60.
update `creature_template` set `maxlevel` = 60 where `maxlevel` > 60 and `maxlevel` not in (61,62,63);
